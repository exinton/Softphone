...
public class Example_5Listener implements SipListener{
  ...
  public Example_5Listener(int port,Example_5GUI GUI)  {
...
public void processResponse(ResponseEvent responseReceivedEvent) {
  ...
  myGUI.display("<<< "+myResponse.toString());
  CSeqHeader cSeqHeader=(CSeqHeader) myResponse.getHeader("CSeq");
...
switch (inputType) {
  ...
  case TERMINATE:
    ...
    Request myByeRequest = myDialog.createRequest("BYE");
    myByeRequest.addHeader(myContactHeader);
    myClientTransaction= mySipProvider.getNewClientTransaction(myByeRequest);
    myDialog.sendRequest(myClientTransaction);

    myGUI.display(">>> "+myByeRequest.toString());
    myGUI.showStatus("Status: "+ myDialog.getState().toString());
  }
