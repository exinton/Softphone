...
public class ShootistAuth implements SipListener {
	...
	public Request processResponseAuthorization(Response response, URI uriReq) {
		...
		try {
			...
			String schema = ((Authorization) (response
					.getHeader(SIPHeaderNames.AUTHORIZATION))).getScheme();
			String nonce = ((Authorization) (response
					.getHeader(SIPHeaderNames.WWW_AUTHENTICATE))).getNonce();
			AuthorizationHeader Authorizationheader = headerFactory
			...
			digest.initialize(realm, USER_AUTH, uriReq.toString(), nonce,
					...
					"MD5");
			System.out.println("Authorization Response antes de modificarlo : "
					+ Authorizationheader.getResponse());
