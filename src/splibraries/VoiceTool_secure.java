...
public void startMedia(String peerIP,int peerPort,int recvPort, int fmt) {
  ...
  try{
   ...
   //We create a SessionManager
    ...
    //create the local endpoint for the local interace on any localp port
    //We initialize the session
    SessionAddress senderAddrs = new SessionAddress();
    
    SessionAddress[] senderAddrset = {senderAddrs};
    SourceDescription sourceDescription = new SourceDescription(0,"jmfSecurity",3,true);
    SourceDescription[] sourceDescriptions = {sourceDescription};
    
    SessionAddress localAddr =
        new SessionAddress (InetAddress.getLocalHost(),recvPort,InetAddress.getLocalHost(),recvPort + 1);
    
   byte[] keys = "abcd01234".getBytes();
    EncryptionInfo encryp = new EncryptionInfo(3,keys );
    
    
    //initialize the rtp manager
    rtpManager.initialize(senderAddrset,sourceDescriptions,1,1,encryp);
