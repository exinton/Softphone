...
public class SipServerSecureListener implements SipListener{
...
SipServerSecureGUI myGUI;

private HashMap locationService;
private ArrayList transactionContext;


  public SipServerSecureListener(String domain, int port, SipServerSecureGUI GUI, boolean recroute)  {
	  SipServerConfiguration.initialUserCredentials(); //set some user credentials to do user authentication
      ...
      myListeningPoint = mySipStack.createListeningPoint(myIP, myPort, transport);  //modify to tls
...
public void processRequest(RequestEvent requestReceivedEvent) {
  ...
  try{
    if (method.equals(Request.REGISTER)) {
	  ...
	  Response response = myMessageFactory.createResponse(Response.TRYING, myRequest);
	  myServerTransaction.sendResponse(response);
	  
	  myGUI.displayClient(response.toString());
	  dsam = new DigestServerAuthenticationHelper();
	  //get userID
	  FromHeader myFH = (FromHeader)myRequest.getHeader(FromHeader.NAME);
	  javax.sip.address.Address messageFromAddress=myFH.getAddress();
	  SipURI fromURI=(SipURI) messageFromAddress.getURI();
	  String userName = fromURI.getUser();
	  //get password or hashed password: here we use password in plaintext
	  
	  String pasword = myconfig.getPassWord(userName);

	  //String hashedCredential = getHashedCredential(userName);
	  //if(!dsam.doAuthenticateHashedPassword(myRequest, hashedCredential)){   //this for password stored in hashed value
	  /*if(pasword == null)
	  {
		  
	  }
	  else */if (!dsam.doAuthenticatePlainTextPassword(myRequest, pasword)) {
		  System.out.println("proxy: send unauthorized");
          Response challengeResponse = myMessageFactory.createResponse(
                  Response.PROXY_AUTHENTICATION_REQUIRED, myRequest);
        		  //Response.UNAUTHORIZED, myRequest);
          System.out.println("proxy: "+ challengeResponse.toString());
          dsam.generateChallenge(myHeaderFactory, challengeResponse, myDomain);
          myServerTransaction.sendResponse(challengeResponse);
          myGUI.displayClient(challengeResponse.toString());
	  ...
	  {
		  System.out.println("Proxy: authentication succeed!");
     ...
     ViaHeader vH = myHeaderFactory.createViaHeader(myIP, myPort,transport, null); //modify to tls
